...
class RoundRectDrawable(backgroundColor: ColorStateList, radius: Float) : Drawable() {
    ...
    private fun buildShadowCorners() {
        ...
        mCornerShadowPaint.shader = RadialGradient(
            0f, 0f, mCornerRadius + mShadowSize,
        ...
        mEdgeShadowPaint.shader = LinearGradient(
            ...
            intArrayOf(Color.TRANSPARENT, Color.TRANSPARENT, Color.TRANSPARENT), floatArrayOf(0f, .5f, 1f),
            Shader.TileMode.CLAMP
        )
        mEdgeShadowPaint.isAntiAlias = false
    }
    ...
    val minHeight: Float
        get() {
            val content = 2 * Math.max(
                mRawMaxShadowSize, mCornerRadius + mInsetShadow
